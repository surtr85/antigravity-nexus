...
export function wordSet(text: string): Set<string> {
  ...
  for (const w of text.toLowerCase().split(/[^\p{L}\p{N}]+/u)) {
    ...
    if (w.length >= 2 || (w.length === 1 && /^\d$/.test(w))) words.add(w);
  }
  return words;
}
 ...
 */
export function wordContainment(a: string, b: string): number {
  const wa = wordSet(a);
  const wb = wordSet(b);
  if (wa.size === 0 || wb.size === 0) return 0;
...
export type DedupDecision =
  | { action: "created"; reason: string }
  | { action: "updated"; reason: string }
...
export function findBestCandidate(
  ...
  for (const memory of candidates) {
    if (memory.status === "deprecated" || memory.status === "superseded") continue;
...
export function decideDedup(candidate: DuplicateCandidate, input: DedupContext): DedupDecision {
  ...
  if (titleSim >= 0.95) {
    ...
    return {
      action: "conflict",
      reason:
        `Same title but very different content (content similarity ${contentSim.toFixed(2)}). ` +
  ...
  if (titleSim >= 0.7) {
    ...
    if (input.project !== undefined && memory.project === input.project && memory.type === input.type) {
      return {
        action: "conflict",
        reason:
          `Same concept in same project/type but divergent content (title ${titleSim.toFixed(2)}, content ${contentSim.toFixed(2)}). ` +
...
export function mergeContent(existing: string, fresh: string): string {
  ...
  let overlap = 0;
  for (const w of frWords) if (exWords.has(w)) overlap++;
  const frRatio = frWords.size === 0 ? 0 : overlap / frWords.size;
  if (frRatio >= 0.8) return ex;
  if (fr.length > ex.length) return fr;
  return ex;
}
 ...
 */
export function detectContradictions(memories: Memory[]): ContradictionPair[] {
  const pairs: ContradictionPair[] = [];
  for (let i = 0; i < memories.length; i++) {
    const a = memories[i];
    if (!a || a.status === "deprecated" || a.status === "superseded") continue;
    for (let j = i + 1; j < memories.length; j++) {
      const b = memories[j];
      if (!b || b.status === "deprecated" || b.status === "superseded") continue;
      if (a.project !== b.project) continue;
      if (a.content.trim().length < 20 || b.content.trim().length < 20) continue;
