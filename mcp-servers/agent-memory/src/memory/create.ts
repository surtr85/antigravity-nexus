...
export function normalizeRelated(ctx: MemoryContext, ids: string[] | undefined): string[] {
  if (!ids) return [];
...
export function relatedSection(
  ctx: MemoryContext,
  relatedIds: string[],
): { text: string; wikilinks: Array<{ label: string }> } {
  if (relatedIds.length === 0) return { text: "", wikilinks: [] };
  const titleStmt = ctx.db.raw.prepare("SELECT title FROM memories WHERE id = ?");
  const lines: string[] = ["## Related", ""];
  const wikilinks: Array<{ label: string }> = [];
  for (const id of relatedIds) {
    const row = titleStmt.get(id) as { title: string } | undefined;
    if (row) {
...
export async function rememberMemory(ctx: MemoryContext, input: RememberInput): Promise<RememberResult> {
  ...
  if (candidate) {
    ...
    decision = { action: "created" as const, reason: "No strong duplicate found." };
  }
  ...
  if (decision.action !== "created" && candidate) {
    ...
    const path = (ctx.db.raw.prepare("SELECT path FROM memories WHERE id = ?").get(existing.id) as { path: string }).path;
  ...
  const discovered = candidates
    .filter(
      (c) =>
        titleSimilarity(c.title, title) >= 0.3 &&
...
async function fetchFullCandidates(ctx: MemoryContext, results: SearchResult[]): Promise<Memory[]> {
  const out: Memory[] = [];
