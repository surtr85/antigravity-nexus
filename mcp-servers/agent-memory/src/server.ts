...
export function createServer(ctx: MemoryContext): McpServer {
...
function registerResources(server: McpServer, ctx: MemoryContext): void {
  server.registerResource(
    "stats",
    "memory://stats",
    { mimeType: "application/json", title: "Memory statistics" },
    () => ({
  ...
  server.registerResource(
    "recent",
    "memory://recent",
    { mimeType: "application/json", title: "Most recently updated memories" },
    () => {
      const rows = ctx.db.raw
        .prepare("SELECT id, title, type, status, updated_at FROM memories ORDER BY updated_at DESC LIMIT 20")
        .all() as Array<{ id: string; title: string; type: string; status: string; updated_at: string }>;
      return {
  ...
  server.registerResource(
    "projects",
    "memory://projects",
    { mimeType: "application/json", title: "Memory count per project" },
    () => {
      const rows = ctx.db.raw
  ...
  server.registerResource(
    "memory",
    "memory://memory/{id}",
    { mimeType: "application/json", title: "A single memory by id" },
    (uri) => {
      ...
      messages: [
        {
          ...
          content: {
            type: "text" as const,
            text:
              `You have a persistent long-term memory system (agent-memory) backed by an Obsidian vault. ` +
      ...
      messages: [
        {
          ...
          content: {
            type: "text" as const,
            text:
              `Session is ending. Review what happened and decide what is worth remembering:\n` +
