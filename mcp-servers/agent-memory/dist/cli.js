...
const COMMANDS = ["reindex", "stats", "doctor", "search"];
async function main() {
...
async function runDoctor(vaultPath, dbPath, autoFix = false) {
    const result = { checks: [], problems: [] };
    const add = (name, ok, detail) => {
        result.checks.push({ name, ok, detail });
        if (!ok)
            result.problems.push(`${name}${detail ? `: ${detail}` : ""}`);
    };
    ...
    try {
        ...
        writable = false;
        add("vault writable", false, err.message);
    }
    if (writable)
        add("vault writable", true);
    let db;
    try {
        ...
        add("database accessible", false, err.message);
        printDoctor(result);
        return 1;
    }
    ...
    add("index matches vault", indexedCount === noteCount, indexedCount === noteCount ? undefined : `index has ${indexedCount}, vault has ${noteCount} notes — run memory:reindex`);
    ...
    for (const note of scan.notes) {
        ...
        if (detectSecrets(blob).length > 0)
            secretNotes.push(note.memory.id);
        if (detectInstructionText(blob))
            instructionNotes.push(note.memory.id);
    }
...
main().catch((err) => {
    logger.error(`cli failed: ${err.stack ?? String(err)}`);
    process.exit(1);
});
