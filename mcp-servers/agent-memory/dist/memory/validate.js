...
export function assertSafeContent(text, field) {
    const hits = detectSecrets(text);
    if (hits.length > 0) {
        const kinds = [...new Set(hits.map((h) => h.kind))].join(", ");
        throw new ContentSafetyError(`${field} appears to contain a secret (${kinds}). ` +
...
export function assertValidMemoryInput(input) {
    if (!z.enum(MEMORY_TYPES).safeParse(input.type).success) {
        throw new ContentSafetyError(`Invalid memory type '${input.type}'. Valid types: ${MEMORY_TYPES.join(", ")}`);
    }
    const title = input.title.trim();
    if (title === "")
        throw new ContentSafetyError("Title must not be empty");
    ...
    if (input.confidence !== undefined && !z.enum(CONFIDENCE_LEVELS).safeParse(input.confidence).success) {
        throw new ContentSafetyError(`Invalid confidence '${input.confidence}'. Valid: ${CONFIDENCE_LEVELS.join(", ")}`);
    }
    if (input.status !== undefined && !z.enum(MEMORY_STATUSES).safeParse(input.status).success) {
        throw new ContentSafetyError(`Invalid status '${input.status}'. Valid: ${MEMORY_STATUSES.join(", ")}`);
    }
 ...
 */
export function detectInstructionText(text) {
