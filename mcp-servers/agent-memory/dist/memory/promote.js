...
export function promoteMemory(ctx, input) {
    const targetType = input.targetType;
    const targetRank = EVOLUTION_CHAIN.indexOf(targetType);
    if (targetRank === -1) {
        throw new ContentSafetyError(`targetType must be one of: ${EVOLUTION_CHAIN.join(", ")}`);
    }
    ...
    for (const id of input.ids) {
        ...
        if (!memory)
            throw new ContentSafetyError(`Source memory not found: ${id}`);
        sources.push(memory);
    }
    if (sources.length === 0) {
        throw new ContentSafetyError("At least one source memory is required");
    }
    const chainRanks = new Map();
    ...
    for (const source of sources) {
        ...
        if (rank !== undefined && rank >= targetRank) {
            throw new ContentSafetyError(`Cannot promote '${source.title}' (${source.type}) into ${targetType}: ` +
